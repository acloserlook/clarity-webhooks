...
const pong = async (logger) => {
  ...
  try {
    ...
    if (healthy[0] && !healthy[1]) logger.log('Client has become unhealthy!');
    if (healthy[1] && !healthy[0]) logger.log('Client has become healthy!');
    if (!healthy[0] && !healthy[1]) logger.log('Client is still unhealthy!');
  }
};
...
const accept = async conversationId =>
  conversationApi(conversationId, undefined, 'accept');
